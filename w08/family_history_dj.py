...
def count_marriages(marriages_dict, people_dict):
    """
    Count the number of marriages per person.

    Parameters:
        marriages_dict: dictionary of marriage data.
            Each item in the dictionary is in this format:
            marriage_key: [husband_key, wife_key, wedding_year]
        people_dict: dictionary of person data.
            Each item in the dictionary is in this format:
            person_key: [name, gender, birth_year, death_year]
    Return:
        None
    """
    marriages_count_list = []
    for _, value in marriages_dict.items():
        ...
        marriages_count_list.append(husband_name)
        marriages_count_list.append(wife_name)

    counter = Counter(marriages_count_list)
    ...
    top_name_index = 0
    top_count_index = 1
    print(most_common[0][top_name_index], most_common[0][top_count_index])
...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for _, value in people_dict.items():
        print(
            value[NAME_INDEX],
            value[DEATH_YEAR_INDEX] - value[BIRTH_YEAR_INDEX],
            value[BIRTH_YEAR_INDEX],
            value[DEATH_YEAR_INDEX],
        )
...
def print_marriages(marriages_dict, people_dict):
    ...
    for _, value in marriages_dict.items():
        husband_name = people_dict[value[HUSBAND_KEY_INDEX]][NAME_INDEX]
        husband_age = (
            value[WEDDING_YEAR_INDEX]
            - people_dict[value[HUSBAND_KEY_INDEX]][BIRTH_YEAR_INDEX]
        )
        wedding_year = value[WEDDING_YEAR_INDEX]
        wife_name = people_dict[value[WIFE_KEY_INDEX]][NAME_INDEX]
        wife_age = (
            value[WEDDING_YEAR_INDEX]
            - people_dict[value[WIFE_KEY_INDEX]][BIRTH_YEAR_INDEX]
        )
        print(
            husband_name,
            husband_age,
