...
def count_genders(people_dict):
    ...
    Return: nothing
    """
    #start the count for genders
    male = 0
    female = 0
    
    print("Genders")

    #process list and count the male and females in list
    for person_key, person_list in people_dict.items():
        gender = person_list[GENDER_INDEX]

        #keeping count of each gender
        if gender == "M":
            male += 1
        else:
            female += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    #start processing through marriages dictionary
    for marriage_key, marriage_list in marriages_dict.items():
        ...
        marriage_year = marriage_list[WEDDING_YEAR_INDEX]

        #cross reference dictionaries for husbands name and birth year
        husband_info = people_dict[husband]
        husband_name = husband_info[NAME_INDEX]
        husband_birth = husband_info[BIRTH_YEAR_INDEX]

        #cross refernce dictionaries for wifes name and birth year
        wife_info = people_dict[wife]
        wife_name = wife_info[NAME_INDEX]
        wife_birth = wife_info[BIRTH_YEAR_INDEX]

        #calculate ages of husband and wife when married
        husband_marriage_age = marriage_year - husband_birth
        wife_marriage_age = marriage_year - wife_birth

        print(f"{husband_name} {husband_marriage_age} > {marriage_year} < {wife_name} {wife_marriage_age}")
